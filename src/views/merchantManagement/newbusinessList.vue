  ...
  <div class="package">
    <div class="hide">
      <div>
        ...
        <span>商家类型</span>
        <el-select v-model="address_type" style="margin-right: 30px;" placeholder="请选择">
      ...
      <el-button type="primary" class v-if="add==1" @click="addBusiness()">新增</el-button>
    ...
    >
      <el-form ref="form" :model="form" :rules="rules" label-width="150px">
        ...
        <el-form-item label="店铺地址">
          ...
          <el-select v-model="form.city_code" disabled @change="cityCode" placeholder="请选择市">
            <el-option
              v-for="item in ProvinceCity"
              :key="item.key"
              :label="item.val"
          ...
          <el-select v-model="form.district_code" disabled @change="areaCode" placeholder="请选择区">
            <el-option
              v-for="item in ProvinceCity"
              :key="item.key"
              :label="item.val"
          ...
          <el-select v-model="form.street_code" @change="streetCode" placeholder="请选择乡镇">
            <el-option v-for="item in street" :key="item.key" :label="item.val" :value="item.key" />
          </el-select>
          <el-select v-model="form.community_code" @change="areaCode" placeholder="请选择村">
            <el-option
        ...
        <el-form-item label="所属乡镇">
          <el-input v-model="form.town_detail" class="input" placeholder="请输入所属乡镇" />
        </el-form-item>
        ...
        <el-form-item label="营业时间">
          <el-radio v-model="form.lineDownTime" :label="'1'">
          ...
          <el-radio v-model="form.lineDownTime" :label="'2'">24小时</el-radio>
        </el-form-item>
        ...
        <el-form-item label="店铺身份" v-else prop="type">
          <div style="display: flex;align-items: center;" v-if="form.type==3">
            <el-form-item label="自提点提成比" label-width="100px">
              <el-input v-model="form.self_percent" class="selectinput" placeholder="%" />
            </el-form-item>
          </div>
          <el-checkbox-group v-model="checkList" style="margin-top: 10px;" v-if="form.type==2">
            <el-checkbox label="线下购商家" style="display: flex;align-items: flex-start;">线下购商家</el-checkbox>
            <div
              style="display: flex;flex-direction: column;height: 50px;justify-content: space-around;margin-left: 23px;"
              class="tong"
            >
...
export default {
  ...
  data() {
    ...
    const user_nameNumber = (rule, value, callback) => {
      ...
      if (this.form.user_name && this.form.user_name.length > 0) {
        callback();
      } else {
  ...
  methods: {
    ...
    startend(val) {
      ...
      if (val) {
        ...
        this.form.contract_end_time = turnDate(val[0], 1);
      }
    },
    uploadbase(img, ext) {
      const data = { img, ext };
