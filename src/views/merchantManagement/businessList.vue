  ...
  <div class="package">
    <div class="hide">
      ...
      <div>
        ...
        <el-input
          ...
          placeholder="名称/ID/手机号"
        />
      </div>
      <el-button type="primary" class v-if="add == 1" @click="addBusiness()"
        >新增</el-button
      >
      <el-button type="danger" class v-if="del == 1" @click="delBusiness()"
        >删除</el-button
      >
      <el-button type="success" class @click="(page = 1), getData()"
        >查询</el-button
      >
      <el-button
        v-if="derive == 1"
      ...
      >
        ...
        <el-table-column label="二维码" align="center">
          <template slot-scope="scope">
            <img
              v-bind:src="scope.row.qr_code"
              @click="imgenlarge(scope.row.qr_code)"
            />
          </template>
        </el-table-column>
        ...
        <el-table-column
          align="center"
          prop="identity.identity_percent"
          label="抽成"
        >
          <template slot-scope="scope">
          ...
          <template slot-scope="scope">
            ...
            <div>
              <el-button
                ...
                type="text"
                size="small"
                @click="(pages = 1), journal(scope.row)"
                >操作日志</el-button
              >
            </div>
          </template>
        ...
        <el-form ref="form" :model="form" :rules="rules" label-width="150px">
          ...
          <el-form-item label="店铺类型" prop="resource">
            <el-radio-group v-model="form.type" @change="caidan">
              <el-radio :label="'1'" :disabled="disabledtype"
                >线上商家</el-radio
              >
              <el-radio :label="'2'" :disabled="disabledtype"
                >线下商家</el-radio
              >
              <el-radio :label="'3'" :disabled="disabledtype">自提点</el-radio>
            </el-radio-group>
          </el-form-item>
          ...
          <el-form-item label="营业时间">
            ...
            <el-time-picker v-model="form.open_time" class="selectinput"></el-time-picker>
            <el-time-picker v-model="form.close_time" class="selectinput"></el-time-picker>
          </el-radio>
            <el-radio v-model="form.time_type" :label="'2'">24小时</el-radio>-->

            <el-radio-group v-model="form.time_type">
              <el-radio :label="1">
                <el-time-picker v-model="form.open_time" class="selectinput" />
                <el-time-picker v-model="form.close_time" class="selectinput" />
              </el-radio>
              ...
              <div class="shuang">
                ...
                <div class="xiong">
                  <el-form-item label="抽成比">
                    <el-input
                      v-model="form.service_percent"
                      class="user_id"
                      placeholder="%"
                    />
                  </el-form-item>
                  <el-checkbox v-model="form.can_coupon"
                    >是否可使用优惠券</el-checkbox
                  >
                </div>
              </div>
            </el-radio>
          ...
          <el-form-item label="店铺身份" v-else prop="type">
            <div
              style="display: flex; align-items: center"
              v-if="form.type == 3"
            >
              <el-form-item label="自提点提成比" label-width="100px">
                <el-input
                  v-model="form.self_percent"
                  class="selectinput"
                  placeholder="%"
                />
              </el-form-item>
            ...
            >
              ...
              <div
                ...
                class="tong"
              >
                <el-radio v-model="form.func" :label="'2'">优惠买单</el-radio>
                <el-radio v-model="form.func" :label="'1'">纯展示</el-radio>
              </div>
              <el-checkbox label="外卖商家">外卖商家</el-checkbox>
            </el-checkbox-group>
            <div
              v-if="form.type == 2"
              style="
              ...
              "
            >
              <div style="width: 100%; font-size: 14px; margin-bottom: 12px">
                ...
                <el-checkbox v-model="form.partner_retail_status">
                  合伙人
                  <el-input
                    v-model="form.waimai_join_par_percent"
                ...
                <el-checkbox v-model="form.village_retail_status">
                  ...
                  <el-input
                    v-model="form.waimai_join_vill_percent"
                ...
                <span style="font-size: 12px; margin-left: 20px"
                  >非必填，若设置提成，则商家所属乡地区合伙人/负责人按该处的提成比进行提成</span
                >
              </div>
              <div style="width: 100%; font-size: 14px; margin-bottom: 12px">
                <span style="width: 90px; display: inline-block"
                ...
                <el-checkbox v-model="form.village_rake_back_status">
                  <el-input
                    v-model="form.waimai_fans_vill_percent"
                ...
                <span style="font-size: 12px; margin-left: 20px"
                  >非必填，若设置返佣，则所有负责人按该提成进行粉丝购物返佣金</span
                >
              </div>

              <div style="width: 100%; font-size: 14px" v-if="form.type == 2">
                <span style="width: 90px; display: inline-block"
                  >推荐人返佣：</span
                >
                <el-checkbox
                  v-model="form.referer_retail_status"
                  :disabled="form.take_out == 1 ? false : true"
                >
                  <el-input
                    v-model="form.referer_user"
                    :disabled="!form.referer_retail_status"
                    class="selectinput"
                    placeholder="请输入推荐人账号"
                  />
                  <el-input
                    v-model="form.referer_retail_precent"
                    :disabled="!form.referer_retail_status"
                    class="selectinput"
                    placeholder="请输入返佣比"
                  />%
          ...
          <el-form-item label="店铺顶图">
            <span style="color: red"
              >*尺寸：710*490为了保证完美的展示您的店铺,请按尺寸上传图片</span
            >
            <Upimg
              :exporteurl="exporteurls"
              :fileList="bg_img_list"
              fileName="image"
              :showList="true"
              :fileHeaders="headers"
              :disabled="false"
          ...
          <el-form-item label="图文详情">
            <div>
              <el-button type="primary" @click="addimg" plain
                >添加图片</el-button
              >
              <el-button type="primary" @click="addtext" plain
                >添加描述</el-button
              >
            </div>
    ...
    >
      ...
      >
        ...
        <bm-local-search :keyword="localization" :auto-viewport="true" />
      </baidu-map>
    </el-dialog>
    ...
    >
      <div style="height: 500px; overflow-y: auto">
        <!-- 表格 -->
        <template>
          <el-table :data="journalList" border style="width: 100%">
            <el-table-column
              align="center"
              prop="admin_user_name"
              label="账号"
            />
            <el-table-column align="center" prop="log_after" label="操作内容">
              <template slot-scope="scope">
                <div>{{ scope.row.log_info }}</div>
                <div v-for="(item, index) in scope.row.log_after" :key="index">
                  {{ item }}
                </div>
              </template>
            </el-table-column>
...
import { upimg1 } from "@/api/user";
import qqMap from "@/components/qqMap";
import settings from "@/settings";
export default {
  // import引入的组件需要注入到对象中才能使用
  components: { Upimg, qqMap },
  // 过滤器
  filters: {
    identity(val) {
  ...
  data() {
    // 检验 表单
    const user_nameNumber = (rule, value, callback) => {
      if (this.form.user_tel && this.form.user_tel.length > 0) {
        callback();
      } else {
    ...
    return {
      ...
      rules: {
        user_name: [
          { required: true, trigger: "blur", validator: user_nameNumber },
        ],
  ...
  methods: {
    ...
    streetCode(val) {
      this.form.community_code = ""; // 村
      this.Query("community", val);
    },
    longitude() {
      /** 详细地址 */
      if (this.form.address && this.form.address.length > 0) {
        this.localization = this.form.address;
    ...
    changeCheckbox(val) {
      if (this.checkList.indexOf("外卖商家") == -1) {
        this.form.take_out = 0;
      } else {
        this.form.take_out = 1;
      }
    },
    radioChange(val) {
    ...
    Degrees(type) {
      ...
      console.log(type);
      this.form.latitude = type.lat;
      this.form.longitude = type.lng;
      this.centers = type;
    ...
    /** 跳转商家 */
    Jump(row) {
      jumpstore({ phone: row.user_tel }).then((res) => {
        console.log(settings.LinkPath);
        let jump_target = "http://192.168.1.34:9530";
        if (res.code === 1) {
          switch (settings.LinkPath) {
            case "https://devadminnew-api.jiunidu.com":
              jump_target = "https://devstore.jiunidu.com";
              break;
            case "https://preadminnew-api.jiunidu.com":
              jump_target = "https://prestore.jiunidu.com";
              break;
            case "https://adminnew-api.jiunidu.com":
              jump_target = "https://store.jiunidu.com";
              break;
          }
          const url = `${jump_target}/tongshuangxiong?token=${res.token}&jump_phone=${res.data.jump_phone}&jump_sign=${res.data.jump_sign}&jump_time=${res.data.jump_time}`;
          console.log(url);
          exportFile(url);
        } else if (res.code === 2) {
          this.$message.error(res.msg);
        ...
        businessAjaxQuery(data).then((res) => {
          ...
          if (res.code == 1) {
            if (type == "category") {
              ...
              if (res.data) {
                if (res.data.agent) {
                  that.ProvinceCity = res.data.agent;
                  that.form.province_code = res.data.agent[0].key;
                  that.form.district_code = res.data.agent[2].key;
                  that.form.city_code = res.data.agent[1].key;
                }
                that.street = res.data.street;
                that.community = res.data.community;
              }
            } else if (type == "agent") {
              //  第一步
      ...
      return new Promise((resolve, reject) => {
        businessGetEdit(data).then((res) => {
          ...
          if (res.code == 1) {
            ...
            that.time_type = res.data.time_type;
            that.contract = [
              res.data.contract_start_time,
              res.data.contract_end_time,
            ];
            if (res.data.bg_img.length > 0) {
              that.bg_img_list = [
                {
                  imgPath: res.data.bg_img,
                  name: "",
                  url: res.data.bg_img_url,
                },
              ];
            }
            /** 经纬度 */
            if (
              res.data.longitude &&
              res.data.longitude.length > 0 &&
              res.data.latitude &&
              res.data.latitude.length > 0
            ) {
              that.centers = {
            ...
            that.form.referer_retail_status =
              res.data.referer_retail_status == 1 ? true : false;
            (that.form.type = res.data.type + ""), // 店铺类型 【1=》线上 2=》线下】
              ...
              (that.form.time_type = res.data.time_type), // 线下营业时长 是全天  还是时段
            ...
            that.Query("category", that.form.type);
            that.Query("agent");
            that.Query("community", that.form.street_code);
    ...
    onSubmit() {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          if (that.bg_img_list.length > 0) {
            that.form.bg_img = that.bg_img_list[0].imgPath;
          } else {
            that.form.bg_img = "";
          }
          let textimg = [];
          ...
          let data = {
            ...
            self_percent: that.form.self_percent,
            ...
            street_code: that.form.street_code,
            community_code: that.form.community_code,
            bg_img: that.form.bg_img,
            waimai_join_par_percent: that.form.waimai_join_par_percent,
            waimai_join_vill_percent: that.form.waimai_join_vill_percent,
            waimai_fans_vill_percent: that.form.waimai_fans_vill_percent,
            partner_retail_status: that.form.partner_retail_status,
            village_retail_status: that.form.village_retail_status,
            village_rake_back_status: that.form.village_rake_back_status,
            referer_user: that.form.referer_user,
            referer_retail_status: that.form.referer_retail_status,
            referer_retail_precent: that.form.referer_retail_precent,
            user_tel: that.form.user_tel,
          };
          if (that.elsec == 1) {
            that.commitAdd(data);
          } else {
    ...
    exporte() {
      const url = `${exporteurl}${storeexport}?name=${this.keyword}&agent_id=${
        this.agent_id
      }&category_id=${
        this.address_type
      }&add_time_order=desc&recommend_order=desc&token=${localStorage.getItem(
        "Token"
      )}`;
      exportFile(url);
    },
      ...
      return new Promise((resolve, reject) => {
        businessList(data).then((res) => {
          ...
          if (res.code == 1) {
            ...
            // that.Query("category", that.form.type);
            that.Query("area");
            that.businessType = res.data.user_identity_data;
