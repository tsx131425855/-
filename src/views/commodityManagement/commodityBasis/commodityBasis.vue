...
<template>
  <div class="package">
    <div class="hide">
      ...
      <el-button plain class @click="change">转入公共库</el-button>
...
import {
  productwareList,
  productwareturnpublic,
  productwareturnrecycle,
  productwareView
} from "@/api/commodity";
...
export default {
  ...
  methods: {
      ...
      if (this.multipleSelection.length > 0) {
        this.$confirm("此操作将会把商品回收, 是否继续?", "提示", {
          ...
          .then(() => {
            ...
            productwareturnrecycle(data).then(res => {
              if (res.code == 1) {
                this.page = 1;
                this.getDatalist();
    ...
    commitpublic() {
      ...
      productwareturnpublic(data).then(res => {
        if (res.code == 1) {
          this.page = 1;
          this.getDatalist();
          this.dialogVisible = false;
