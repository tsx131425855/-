...
<template>
  <div class="package">
    <div class="hide">
      ...
      <el-button type="info" @click="See()">查看</el-button>
      <el-button type="warning" @click="change">转入公共库</el-button>
    ...
    >
      <div slot="forms">
        ...
        <router-link class="routeLike" :to="{ path: `/couponDetail?coupon_id=1`}">无分类,前往设置</router-link>
...
import {
  productwarerecycleindex,
  productwarerecycleturnpublic,
  productwarerecycleview
} from "@/api/commodity";
...
export default {
  ...
  methods: {
    ...
    commitpublic() {
      ...
      productwarerecycleturnpublic(data).then(res => {
        if (res.code == 1) {
          this.page = 1;
          this.getDatalist();
          this.dialogVisible = false;
    ...
    getDatalist() {
      ...
      return new Promise((resolve, reject) => {
        productwarerecycleindex(data).then(res => {
          this.loading = false;
          if (res.code == 1) {
