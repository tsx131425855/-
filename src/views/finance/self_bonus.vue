  ...
  <div class="package">
    ...
    <template>
      ...
      >
        ...
        <el-table-column align="center" prop="income_id" label="序号" width="70" />
        <el-table-column align="center" prop="update_time_str" label="到账时间" />
        <el-table-column align="center" prop="store.self_address.self_name" label="自提点名称" />
        <el-table-column align="center" prop="store_user.user_tel" label="用户账号" />
        <el-table-column align="center" prop="income" label="分成金额" />
        <!-- <el-table-column align="center" prop="last_ip" label="分红项"></el-table-column> -->
        <el-table-column align="center" prop="order_sn" label="交易订单号" />
...
// 提现明细
/* eslint-disable */
import { self_bonus, exportSelfBonus } from "@/api/finance";
  ...
  methods: {
    ...
    exportExcel() {
      ...
      this.$confirm("请确认筛选条件是否符合", "提示", {
        ...
        .then(() => {
          ...
          //开启新下载窗口
          window.open(exportSelfBonus(data));
        })
        .catch(res => {
          console.log(res);
        });
    ...
    saveData(bool) {
      ...
      if (bool) {
        // 为true时为保存， 反之取出
        let data =
          JSON.parse(sessionStorage.getItem("self_bonus_condition")) || {};
        ...
        sessionStorage.setItem("self_bonus_condition", JSON.stringify(data));
      } else {
        // 取出缓存中的条件数据
        let data = JSON.parse(sessionStorage.getItem("self_bonus_condition"));
