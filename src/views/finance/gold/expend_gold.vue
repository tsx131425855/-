  ...
  <div class="package">
    <div class="hide hide_title">
      ...
      <el-dropdown @command="handleCommand">
        <el-button type="primary">
          {{ type_list[screen_condition] || '请选择' }}
    ...
    <template>
      ...
      >
        ...
        <el-table-column align="center" prop="coin_type_name" label="交易类型" />
        <el-table-column align="center" label="金额">
          <template
            slot-scope="scope"
          >{{ (scope.row.coin_type_sign === 'del' ? '-' : '+') + scope.row.num }}</template>
        </el-table-column>
...
// 提现明细
/* eslint-disable */
import { user_gold, exportUserGold } from "@/api/finance";
    ...
    getData() {
      ...
      return new Promise((resolve, reject) => {
        user_gold(data).then(res => {
          if (res.code == 1) {
            that.dataList = res.data.list.data.list;
            that.total = res.data.list.data.total;
            that.type_list = res.data.list.type;
            that.loading = false;
            if (res.action) {
              res.action.map(val => {
 ...
 if(res.agent_map){
    ...
    saveData(bool) {
      ...
      if (bool) {
        // 为true时为保存， 反之取出
        let data =
          JSON.parse(sessionStorage.getItem("expend_gold_condition")) || {};
        ...
        sessionStorage.setItem("expend_gold_condition", JSON.stringify(data));
      } else {
        // 取出缓存中的条件数据
        let data = JSON.parse(sessionStorage.getItem("expend_gold_condition"));
  ...
  created() {
    ...
    this.search_section = JSON.parse(this.$route.query.time);
    this.$route.meta.title = this.$route.query.user_name + " - 金币支出";
    this.saveData();
    this.getData();
  },
  //生命周期 - 挂载完成（可以访问DOM元素）
