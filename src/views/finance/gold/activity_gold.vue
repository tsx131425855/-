...
if(res.agent_map){
    ...
    saveData(bool) {
      ...
      if (bool) {
        // 为true时为保存， 反之取出
        let data =
          JSON.parse(sessionStorage.getItem("activity_gold_condition")) || {};
        ...
        sessionStorage.setItem("activity_gold_condition", JSON.stringify(data));
      } else {
        // 取出缓存中的条件数据
        let data = JSON.parse(
          sessionStorage.getItem("activity_gold_condition")
        );
  ...
  created() {
    ...
    this.$route.meta.title = this.$route.query.user_name + " - 可用金币";
