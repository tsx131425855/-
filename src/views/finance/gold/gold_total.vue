...
export default {
  ...
  methods: {
    ...
    saveData(bool) {
      ...
      if (bool) {
        // 为true时为保存， 反之取出
        let data =
          JSON.parse(sessionStorage.getItem("gold_total_condition")) || {};
        ...
        sessionStorage.setItem("gold_total_condition", JSON.stringify(data));
      } else {
        // 取出缓存中的条件数据
        let data = JSON.parse(sessionStorage.getItem("gold_total_condition"));
  ...
  created() {
    ...
    this.$route.meta.title = this.$route.query.user_name + " - 金币总额";
    this.saveData();
    this.getData();
  },
  //生命周期 - 挂载完成（可以访问DOM元素）
