  ...
  <div class="package">
    ...
    <template>
      ...
      >
        ...
        <el-table-column align="center" min-width="100" prop="update_time" label="确认收货时间" />
        <el-table-column align="center" min-width="90" prop="order_sn" label="订单号" />
        <el-table-column align="center" prop="name" label="商家名称" />
        <el-table-column align="center" prop="phone" label="用户账号" />
        <el-table-column align="center" prop="user_nickname" label="下单用户昵称" />
        <el-table-column align="center" prop="user_tel" label="用户账号" />
        <el-table-column align="center" prop="amount" label="订单总金额" />
        <el-table-column align="center" prop="service_percent" label="抽成比" />
        <el-table-column align="center" prop="cc_amount" label="抽成金额" />
        <el-table-column align="center" prop="status_msg" label="交易状态" />
...
// 提现明细
/* eslint-disable */
import { platform_pumping, exportPlatformPumping } from "@/api/finance";
  ...
  methods: {
    ...
    getData() {
      ...
      return new Promise((resolve, reject) => {
        platform_pumping(data).then(res => {
          console.log(res);
          if (res.code == 1) {
            ...
            if (res.action) {
              res.action.map(val => {
                if (val.tag == "jndcutexport" && val.display == 1) {
                  that.derive = 1;
                }
              });
            }
    ...
    exportExcel() {
      ...
      this.$confirm("请确认筛选条件是否符合", "提示", {
        ...
        .then(() => {
          ...
          //开启新下载窗口
          window.open(exportPlatformPumping(data));
        })
        .catch(res => {
          console.log(res);
        });
    ...
    saveData(bool) {
      ...
      if (bool) {
        // 为true时为保存， 反之取出
        let data =
          JSON.parse(sessionStorage.getItem("platform_pumping_condition")) ||
          {};
        ...
        sessionStorage.setItem(
          "platform_pumping_condition",
          JSON.stringify(data)
        );
      } else {
        // 取出缓存中的条件数据
        let data = JSON.parse(
          sessionStorage.getItem("platform_pumping_condition")
        );
