...
<template>
  <div class="package">
    <div class="hide hide_title">
      ...
      />
      <el-button type="success" icon="el-icon-search" @click="querySearch">查询</el-button>
    ...
    <template>
      ...
      >
        ...
        <el-table-column align="center" min-width="160" prop="add_time" label="下单日期" />
        <el-table-column fixed align="center" prop="id" min-width="160" label="订单编号" />
        ...
        <el-table-column align="center" prop="property_name" min-width="100" label="楼盘名称" />
        <el-table-column align="center" prop="total_pay_fee" label="认筹金额" />
        <el-table-column align="center" prop="wx_pays" min-width="120" label="微信支付金额" />
        <el-table-column align="center" prop="service_charge" label="手续费" />
        <el-table-column align="center" prop="property_counselor" label="置业顾问" />
        <el-table-column align="center" prop="status_str" label="订单状态" />
        <el-table-column align="center" min-width="160" prop="update_time" label="订单更新时间" />
        <el-table-column align="center" prop="refund_id" min-width="120" label="是否申请售后" />
        <!-- <el-table-column align="center" prop="return_balance" label="退余额"></el-table-column> -->
        <el-table-column align="center" prop="wx_fee" label="退微信" />
        <el-table-column align="center" prop="wx_service_charge" label="退手续费" />
        <!-- <el-table-column align="center" prop="return_gold_coin" label="退金币"></el-table-column> -->
        <el-table-column align="center" prop="extract_pay" label="平台抽成" />
        <el-table-column align="center" prop="one_agent" label="一级代理商抽成" />
        <el-table-column align="center" prop="two_agent" label="二级代理商抽成" />
        <el-table-column align="center" prop="three_agent" label="三级代理商抽成" />
...
// 提现明细
/* eslint-disable */
import { get_registration, exportGetRegistration } from "@/api/finance";
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      this.loading = true;
      const data = {
        time_type: this.screen_condition,
      ...
      return new Promise((resolve, reject) => {
        get_registration(data).then(res => {
          console.log(res);
          if (res.code == 1) {
            that.dataList = res.data.list;
            that.total = res.data.total;
            that.condition_list = res.data.str_status;
            that.screen_list = res.data.time_type;
            that.loading = false;
            if (res.action) {
              res.action.map(val => {
    ...
    exportExcel() {
      ...
      this.$confirm("请确认筛选条件是否符合", "提示", {
        ...
        .then(() => {
          ...
          //开启新下载窗口
          window.open(exportGetRegistration(data));
        })
        .catch(res => {
          console.log(res);
        });
    ...
    saveData(bool) {
      ...
      if (bool) {
        // 为true时为保存， 反之取出
        let data =
          JSON.parse(sessionStorage.getItem("get_registration_condition")) ||
          {};
        ...
        sessionStorage.setItem(
          "get_registration_condition",
          JSON.stringify(data)
        );
      } else {
        // 取出缓存中的条件数据
        let data = JSON.parse(
          sessionStorage.getItem("get_registration_condition")
        );
