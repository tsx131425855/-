  ...
  <div class="package">
    ...
    <template>
      ...
      >
        ...
        <el-table-column align="center" prop="pay_id" label="序号" width="70" />
        ...
        <el-table-column align="center" prop="status" label="交易状态"></el-table-column>
...
// 提现明细
/* eslint-disable */
import { expend_weixin, exportExpendWeixin } from "@/api/finance";
...
export default {
  ...
  methods: {
      ...
      return new Promise((resolve, reject) => {
        expend_weixin(data).then(res => {
          if (res.code == 1) {
            ...
            if (res.action) {
              res.action.map(val => {
                if (val.tag == "payrecordexport" && val.display == 1) {
                  that.derive = 1;
                }
              });
            }
    ...
    exportExcel() {
      ...
      this.$confirm("请确认筛选条件是否符合", "提示", {
        ...
        .then(() => {
          ...
          //开启新下载窗口
          window.open(exportExpendWeixin(data));
        })
        .catch(res => {
          console.log(res);
        });
    ...
    saveData(bool) {
      ...
      if (bool) {
        // 为true时为保存， 反之取出
        let data =
          JSON.parse(sessionStorage.getItem("expend_weixin_condition")) || {};
        ...
        sessionStorage.setItem("expend_weixin_condition", JSON.stringify(data));
      } else {
        // 取出缓存中的条件数据
        let data = JSON.parse(
          sessionStorage.getItem("expend_weixin_condition")
        );
  ...
  created() {
    ...
    this.$route.meta.title = this.$route.query.user_name + " - 微信支出";
    this.saveData();
    this.getData();
  },
  //生命周期 - 挂载完成（可以访问DOM元素）
