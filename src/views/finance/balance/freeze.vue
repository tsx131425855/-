...
// 提现明细
/* eslint-disable */
import { freeze, exportFreeze } from "@/api/finance";
...
export default {
  ...
  methods: {
      ...
      return new Promise((resolve, reject) => {
        freeze(data).then(res => {
          if (res.code == 1) {
            ...
            if (res.action) {
              res.action.map(val => {
                if (val.tag == "blockedrecordexport" && val.display == 1) {
                  that.derive = 1;
                }
              });
            }
    ...
    saveData(bool) {
      ...
      if (bool) {
        // 为true时为保存， 反之取出
        let data = JSON.parse(sessionStorage.getItem("freeze_condition")) || {};
        ...
        sessionStorage.setItem("freeze_condition", JSON.stringify(data));
      } else {
        // 取出缓存中的条件数据
        let data = JSON.parse(sessionStorage.getItem("freeze_condition"));
        if (!data) return; // 如果没有缓存， 则直接退出；

        // 判断用户是否是已同条件进来（ 同条件应属于tab切换导致 ）， 若非同条件。 则不读取缓存数据， 按照默认条件进行操作
  ...
  created() {
    ...
    this.$route.meta.title = this.$route.query.user_name + " - 冻结总额";
    this.saveData();
    this.getData();
  },
  //生命周期 - 挂载完成（可以访问DOM元素）
