...
// 提现明细
/* eslint-disable */
import { expend_balance, exportExpendBalance } from "@/api/finance";
...
export default {
  ...
  methods: {
      ...
      return new Promise((resolve, reject) => {
        expend_balance(data).then(res => {
          if (res.code == 1) {
            ...
            if (res.action) {
              res.action.map(val => {
                if (val.tag == "expenserecordexport" && val.display == 1) {
                  that.derive = 1;
                }
              });
            }
    ...
    exportExcel() {
      ...
      this.$confirm("请确认筛选条件是否符合", "提示", {
        ...
        .then(() => {
          ...
          //开启新下载窗口
          window.open(exportExpendBalance(data));
        })
        .catch(res => {
          console.log(res);
        });
    ...
    saveData(bool) {
      ...
      if (bool) {
        // 为true时为保存， 反之取出
        let data =
          JSON.parse(sessionStorage.getItem("expend_balance_condition")) || {};
        ...
        sessionStorage.setItem(
          "expend_balance_condition",
          JSON.stringify(data)
        );
      } else {
        // 取出缓存中的条件数据
        let data = JSON.parse(
          sessionStorage.getItem("expend_balance_condition")
        );
  ...
  created() {
    ...
    this.$route.meta.title = this.$route.query.user_name + " - 余额支出总额";
    this.saveData();
    this.getData();
  },
  //生命周期 - 挂载完成（可以访问DOM元素）
